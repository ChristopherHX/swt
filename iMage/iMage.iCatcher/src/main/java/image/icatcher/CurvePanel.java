...
public class CurvePanel extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < 255; i++) {
            ...
            if (i != 0) {
                g.drawLine(ox0, oy0, (int) (response[0] * scale), i * scale);
            }
            g.drawArc((int) (response[0] * scale), i * scale, 10, 10, 0, 360);
            ox0 = (int) (response[0] * scale);
            oy0 = i * scale;
            g.setColor(Color.GREEN);
            if (i != 0) {
                g.drawLine(ox1, oy1, (int) (response[1] * scale), i * scale);
            }
            g.drawArc((int) (response[1] * scale), i * scale, 10, 10, 0, 360);
            ox1 = (int) (response[1] * scale);
            oy1 = i * scale;
            g.setColor(Color.BLUE);
            if (i != 0) {
                g.drawLine(ox2, oy2, (int) (response[2] * scale), i * scale);
            }
            g.drawArc((int) (response[2] * scale), i * scale, 10, 10, 0, 360);
